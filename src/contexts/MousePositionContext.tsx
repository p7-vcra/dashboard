...
function MousePositionProvider({ children }: { children: React.ReactNode }) {
    ...
    const [mousePosition, setMousePosition] = useState<LatLng>(new LatLng(denmarkCoords.lat, denmarkCoords.lng));
    ...
    return (
        <MousePositionContext.Provider value={{ mousePosition, setMousePosition: updateMousePosition }}>
            {children}
...
function useMousePosition() {
    ...
    if (!context) {
        throw new Error("useMousePosition must be used within an MousePositionProvider");
    }
    return context;
}
